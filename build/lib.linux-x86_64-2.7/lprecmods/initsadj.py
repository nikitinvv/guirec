...
def create_adj(Pgl,Padj_file,filter_type):
	"initialize parameters for adj transform and save to file"
	Padj=precompute_adj(Pgl,filter_type);
	savePadjparams(Padj,Padj_file);
	print ("Parameters for the adjoint transform were saved to the file %s\n" % Padj_file);
	return Padj;

def precompute_adj(P,filter_type):
...
def savePadjparams(P,P_file):
	...
	fid.write(int32(Nlpids));
	fid.write(int32(Nwids));
	
	for k in range(0,Nspan):
	    fid.write(float32(P.C2lp1[k]));
	for k in range(0,Nspan): 
	    fid.write(float32(P.C2lp2[k]));
	for k in range(0,Nspan): 
	    fid.write(float32(P.lp2p1[k]));
	for k in range(0,Nspan): 
	    fid.write(float32(P.lp2p2[k]));
	for k in range(0,Nspan): 
	    fid.write(float32(P.lp2p1w[k]));
	for k in range(0,Nspan): 
	    fid.write(float32(P.lp2p2w[k]));

	fid.write(int32(P.cids));
	fid.write(int32(P.lpids));
	fid.write(int32(P.wids));
	
	fZvec=ndarray.flatten(transpose(array([real(ndarray.flatten(P.fZgpu)),ndarray.flatten(imag(P.fZgpu))])));
	fid.write(float32(fZvec));
	if(filter!=None):
		fid.write(float32(P.filter));
	fid.close();
